  ...
  const fetchChats = async () => {
    ...
    while (attempts < 3) {
      try {
        ...
        attempts++;
        console.log("Chat fetch attempt", attempts, err);
    ...
    const fetchMessages = async () => {
      ...
      while (attempts < 3) {
        try {
          ...
          if (isMounted) {
            console.log("API response messages:", data.data);
            console.log("Message array length:", data.data?.length || 0);
          ...
          attempts++;
          if (isMounted) {
            console.log("Message fetch attempt", attempts, err);
