...
const EditProfile = () => {
  ...
  const normalizeSkill = (input) => {
    ...
    if (!value) return "";
    return skills.find((skill) => skill.toLowerCase() === value) || "";
  };
  ...
  useEffect(() => {
    if (user) {
      setForm((prevState) => ({
        ...
        skillsProficientAt: Array.isArray(user?.skillsProficientAt) ? user.skillsProficientAt : [],
      ...
      setTechStack(Array.isArray(user?.projects) ? user.projects.map(() => "Select some Tech Stack") : []);
    }
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (uploadedUrl) {
        const nextUser = { ...user, picture: uploadedUrl };
        setUser(nextUser);
  ...
  const handleAddSkill = (e) => {
    ...
    if (name === "skill_to_learn") {
      const resolvedSkill = normalizeSkill(skillsToLearn);
      if (!resolvedSkill) {
        toast.error("Select a skill to add");
        return;
      }
      if (form.skillsToLearn.includes(resolvedSkill)) {
        toast.error("Skill already added");
        return;
      }
      if (form.skillsProficientAt.includes(resolvedSkill)) {
        toast.error("Skill already added in skills proficient at");
        return;
      }
      setForm((prevState) => ({
        ...prevState,
        skillsToLearn: [...prevState.skillsToLearn, resolvedSkill],
      }));
      setSkillsToLearn("");
    } else {
      const resolvedSkill = normalizeSkill(skillsProficientAt);
      if (!resolvedSkill) {
        toast.error("Select a skill to add");
        return;
      }
      ...
      setForm((prevState) => ({
        ...prevState,
        skillsProficientAt: [...prevState.skillsProficientAt, resolvedSkill],
      }));
            ...
            <Tab eventKey="registration" title="Registration">
              ...
              <div className="form-group">
                ...
                <input
                  type="text"
                  className="form-control"
                  list="skills-learn-list-edit"
                  value={skillsToLearn}
                  onChange={(e) => setSkillsToLearn(e.target.value)}
                  placeholder="Search and select a skill"
                />
                <datalist id="skills-learn-list-edit">
                  {skills.map((skill, index) => (
                    <option key={index} value={skill} />
                  ))}
                </datalist>
