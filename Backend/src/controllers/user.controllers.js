...
export const sendScheduleMeet = asyncHandler(async (req, res) => {
  ...
  const message = `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
      ...
      <p><strong>${req.user.name}</strong> (${req.user.username}) has requested a video meeting with you.</p>
      ...
      <p><a href="http://localhost:5173/chats" style="background-color: #3BB4A1; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">Go to Chats</a></p>
      
      <p>Best regards,<br/>SkillXchange Team</p>
    </div>
  `;

  console.log(`Sending meeting email to: ${to} from ${req.user.email}`);
